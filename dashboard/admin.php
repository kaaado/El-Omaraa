        ...
        <div class="card_header">
                    ...
                    if (isset($_POST["add"])) {
                        ...
                        if (!empty($_POST["name"]) && !empty($_POST["email"]) && !empty($_POST["password"]) && !empty($_POST["phone"]) && !empty($_POST["address"]) && !empty($_POST["dateofbirth"])) {
                            ...
                            if ($result->num_rows > 0) {
                                ...
                                $phone_check_query = "SELECT * FROM admin WHERE phone_number=?";
                                ...
                                if ($result->num_rows > 0) {
                                    ...
                                    if (strlen($phone) != 10 || !ctype_digit($phone)) {
                                        ...
                                        $stmt = $conn->prepare("INSERT INTO admin (name, email, password, phone_number, address, dateofbirth) VALUES (?, ?, ?, ?, ?, ?)");
                                        if ($stmt) {
                                            // Bind parameters
                                            ...
                                            // Execute statement
                                            if ($stmt->execute()) {
                                                echo "<script>window.location.pathname = 'Web/dashboard/admin.php';</script>";
                                                exit;
                                            } else {
            ...
            if (isset($_POST["update_patient"])) {
               ...
               if(isset($_POST["password"])) {
                   ...
                   $sql = "UPDATE admin SET name='$name', email='$email', phone_number='$phone_number', address='$address', dateofbirth='$dateofbirth', password='$password' WHERE id='$id'";
               } else {
                   // Update query without password
                   $sql = "UPDATE admin SET name='$name', email='$email', phone_number='$phone_number', address='$address', dateofbirth='$dateofbirth' WHERE id='$id'";
               }
