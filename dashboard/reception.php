...
<section class="home_section">
    ...
    <td><?php echo $i; ?></td>
    <td>
        <form action="reception.php" method="post" class="update-form">
    ...
    <?php if ($_SESSION["USER_Role"] === "admin") { ?>
        <td class="btn-action">
            ...
            <?php } else { ?>
                ...
                <form  method="post">
    <input type="hidden" name="table" value="reception">
    <input type="hidden" name="id" value="<?php echo $row['id']; ?>">
    ...
    <?php } ?>
                    ...
                    echo "<tr><td colspan='7'><p style='text-align:center;'>No reception found</p></td></tr>";
        ...
        <div class="card_header">
                  ...
                  if ($_SERVER["REQUEST_METHOD"] == "POST") {
                    if (isset($_POST["add"])) {
                        ...
                        if (!empty($_POST["name"]) && !empty($_POST["email"]) && !empty($_POST["password"]) && !empty($_POST["phone"]) && !empty($_POST["address"]) && !empty($_POST["dateofbirth"])) {
                            ...
                            $email_check_query = "SELECT * FROM reception WHERE email=?";
                            ...
                            if ($result->num_rows > 0) {
                                ...
                                $phone_check_query = "SELECT * FROM reception WHERE phone_number=?";
                                ...
                                if ($result->num_rows > 0) {
                                    ...
                                    if (strlen($phone) != 10 || !ctype_digit($phone)) {
                                        ...
                                        $stmt = $conn->prepare("INSERT INTO reception (name, email, password, phone_number, address, dateofbirth) VALUES (?, ?, ?, ?, ?, ?)");
                                        if ($stmt) {
                                            // Bind parameters
                                            ...
                                            // Execute statement
                                            if ($stmt->execute()) {
                                                echo "<script>window.location.pathname = 'Web/dashboard/reception.php';</script>";
                                                exit;
                                            } else {
                                                // Redirect to the page with error message
                                                header("location: reception.php?error=1");
                                                exit;
                                            }
                                        } else {
            ...
            if (isset($_POST["update_patient"])) {
               ...
               if(isset($_POST["password"])) {
                   ...
                   $sql = "UPDATE reception SET name='$name', email='$email', phone_number='$phone_number', address='$address', dateofbirth='$dateofbirth', password='$password' WHERE id='$id'";
               } else {
                   // Update query without password
                   $sql = "UPDATE reception SET name='$name', email='$email', phone_number='$phone_number', address='$address', dateofbirth='$dateofbirth' WHERE id='$id'";
               }
